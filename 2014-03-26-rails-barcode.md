...
wicked pdfでsvg形式のbarcodeをPDFに出力するには若干の工夫が必要

imageを出力するときと同じくbase64 encodingが必要

<http://stackoverflow.com/questions/7880079/embedding-svg-in-pdf-using-wicked-pdf-wkhtmltopdf>


`"data:image/svg+xml;base64,#{Base64.encode64(barcode.to_svg)}"`


最終的にimgタグがこのようになるように

<http://depth-first.com/articles/2011/10/19/display-inline-svg-using-the-img-tag/>
